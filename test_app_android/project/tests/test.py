...
class TestChangePlan:
    def __init__(self, data, app_pages):
        ...
        if not pd.isna(data.get('new_options_price_day')):
            ...
            if isinstance(self.price_dtm_day, int):
    ...
    def test_check_info(self, app_pages):
        if self.cycle == 'd' and self.type_case == 'options':
            ...
            cycle = 'm'
        try:
            self.app_pages.check.info_text(self.value_gb, self.value_min, self.price_tp, self.value_gb_old, self.value_min_old, self.price_tp_old,  self.price_dtm, cycle)
        except Exception as e:
            allure.attach(str(e), name="Ошибка", attachment_type=allure.attachment_type.TEXT)
    ...
    def test_wait_tariff(self, app_pages, param):
        if param == 1:
            number = self.value_gb
            ...
            if self.cycle == 'd' and self.type_case == 'options':
                number = float(self.price_tp_day) + float(self.price_dtm_day)
    ...
    def test_ap_tariff(self, app_pages):
        ...
        try:
            self.app_pages.my_products.ap(price, cycle)
        except Exception as e:
            allure.attach(str(e), name="Ошибка", attachment_type=allure.attachment_type.TEXT)
    ...
    # Проверка смены пресета/ подключение услуг
        ...
        if (row['old_gb_value'] != row['new_gb_value']) or (row['old_min_value'] != row['new_min_value']):
            with allure.step(f'Тест-кейс: смена пресета "{row['new_tariff_name']}"'):
                ...
                #Проверка цены на кнопке оплатить и переход далее
                if (row['new_gb_value'] <= row['old_gb_value'] and row['new_min_value'] < row['old_min_value']) and (pd.isna(row['new_options_price']) or not pd.isna(row['new_options_price']) and not pd.isna(row['old_options_price'])):
